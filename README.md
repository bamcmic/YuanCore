...
- ✅ **int 0x80 系统调用**：28 个服务（打印/窗口/按键/文件/鼠标/随机数/节拍睡眠/描点/
  应用注册表/终端显隐），程序与内核解耦
- ✅ **屏幕合成器**：桌面/控制台/窗口/光标四层 z 序，按矩形局部合成，
  光标移动零备份零残影
- ✅ **PS/2 鼠标**：IRQ12 三字节包驱动，屏幕光标（白箭头+投影），
  按钮可点击命中，**窗口可按住标题栏拖动**
- ⚠️ **程序加载执行（实验性，默认禁用）**：ring3 切换路径存在未定位根因的
  稳定性问题（内核态踩踏），为保系统可用已默认关闭；
  调试时 `settings exp exec on` 启用，`run hello` / 开始菜单启动项即恢复
- ✅ **应用注册表 + 开始菜单检索**：打字即过滤应用，Enter 启动
  （ring3 应用启动项当前受上面的实验开关控制）
- ✅ **文件管理器**：目录导航、打开文本文件，键盘/鼠标双通道
- ✅ **可定制桌面**：终端窗口可关闭/重开，任务栏右侧竖放、Win 键唤起/收起
- ✅ **文件系统**：内存文件系统（目录树 + 堆存储），
